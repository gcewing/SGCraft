...
public class SGBaseTE extends BaseTileInventory {
    ...
    static final boolean debugState = false;
    static final boolean debugEnergyUse = false;
    static final boolean debugConnect = false;
    static final boolean debugTransientDamage = false;
    static final boolean debugTeleport = false;
    static final float defaultChevronAngle = 40f;
    static final float[][] chevronAngles = {
            //     0    1    2    <-- Base camouflage level
            {45f, 45f, 40f}, // 7 chevrons
            {36f, 33f, 30f}  // 9 chevrons
    };
    static final Random random = new XSTR();
    static final DamageSource transientDamage = new DamageSource("sgcraft:transient");
    ...
    final IInventory inventory = new InventoryBasic("Stargate", false, numInventorySlots);
    final List<TrackedEntity> trackedEntities = new ArrayList<>();
    ...
    void enterState(SGState newState, int newTimeout) {
        if (debugState)
            System.out.printf("SGBaseTE: at %s in dimension %s entering state %s with timeout %s\n",
                    getPos(), getWorldDimensionId(worldObj), newState, newTimeout);
        ...
        if (isMerged) {
            if (debugState && state != SGState.Connected && timeout > 0) {
                ...
                System.out.printf(
                        ...
                        getPos(), dimension, state, timeout);
            }
            tickEnergyUsage();
            if (timeout > 0) {
...
class BlockRef {
    ...
    public final IBlockAccess worldObj;
    final BlockPos pos;

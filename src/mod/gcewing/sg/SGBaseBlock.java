...
public class SGBaseBlock extends SGBlock<SGBaseTE> {
    ...
    protected static final String[] textures = {"stargateBlock", "stargateRing", "stargateBase_front"};
    protected static final ModelSpec model = new ModelSpec("block/sg_base_block.smeg", textures);
    static final boolean debugMerge = false;
    ...
    void checkForMerge(World world, BlockPos pos) {
        ...
        if (!isMerged(world, pos)) {
            ...
            if (debugMerge)
                System.out.print("SGBaseBlock: Merging\n");
            SGBaseTE te = getTileEntity(world, pos);
            te.setMerged(true);

...
public class BaseTEChunkManager implements ForgeChunkManager.LoadingCallback {
    ...
    public BaseTEChunkManager(BaseMod mod) {
        ...
        if (debug) SGCraft.log.debug(String.format("%s: BaseTEChunkManager: Chunk loading callback installed", base.modPackage));
    }
    ...
    public void ticketsLoaded(List<ForgeChunkManager.Ticket> tickets, World world) {
        if (debug) SGCraft.log.debug(String.format("%s: BaseTEChunkManager.ticketsLoaded for %s", base.modPackage, world));
        for (Ticket ticket : tickets) {
            ...
            if (nbt != null)
                if (nbt.getString("type").equals("TileEntity")) {
                    ...
                    if (debug) SGCraft.log.debug(String.format("%s: BaseTEChunkManager.ticketsLoaded: Ticket for %s at (%d, %d, %d)", base.modPackage, te, x, y, z));
                    if (!(te instanceof BaseTileEntity && reinstateChunkTicket((BaseTileEntity)te, ticket))) {
                        if (debug) SGCraft.log.debug(String.format("%s: BaseTEChunkManager.ticketsLoaded: : Unable to reinstate ticket", base.modPackage));
    ...
    protected void forceChunkRangeOnTicket(BaseTileEntity te, Ticket ticket) {
        ...
        if (debug) SGCraft.log.debug(String.format("BaseChunkLoadingTE: Forcing range (%s,%s)-(%s,%s) in dimension %s", minX, minZ, maxX, maxZ, te.getWorldObj().provider.dimensionId));

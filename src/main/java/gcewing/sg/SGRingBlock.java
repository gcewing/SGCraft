...
//      registerSubItemNames();
    ...
    public void unmergeFrom(World world, BlockPos pos, BlockPos basePos) {
        SGRingTE te = getRingTE(world, pos);
        if (SGBaseBlock.debugMerge) SGCraft.log.debug(String.format("SGRingBlock.unmergeFrom: ring at %s base at %s te.isMerged = %s te.basePos = %s",
    ...
    void updateBaseBlocks(World world, BlockPos pos, SGRingTE te) {
        if (SGBaseBlock.debugMerge) SGCraft.log.debug(String.format("SGRingBlock.updateBaseBlocks: merged = %s, base = %s", te.isMerged, te.basePos));
        for (int i = -2; i <= 2; i++)
            for (int j = -4; j <= 0; j++)
                for (int k = -2; k <= 2; k++) {
                    ...
                    if (block instanceof SGBaseBlock) {
                         if (SGBaseBlock.debugMerge) SGCraft.log.debug(String.format("SGRingBlock.updateBaseBlocks: found base at %s", bp));

    ...
    extends BaseSubsystem implements IGuiHandler
{
    ...
    public <ITEM extends Item> ITEM addItem(ITEM item, String name) {
        ...
        if (debugBlockRegistration) SGCraft.log.debug(String.format("BaseMod.addItem: Registered %s as %s", item, name));
        if (creativeTab != null) {
            if (debugCreativeTabs) SGCraft.log.debug(String.format("BaseMod.addItem: Setting creativeTab of %s to %s", name, creativeTab));
    ...
    {
        SGCraft.log.trace(String.format("%s: BaseMod.addEntity: %s, \"%s\", %s\n", getClass().getSimpleName(), cls.getSimpleName(), name, id));
    ...
    public void openGui(EntityPlayer player, int id, World world, BlockPos pos) {
        ...
        if (debugGui) SGCraft.log.debug(String.format("BaseMod.openGui: for %s with id 0x%x in %s at (%s, %s, %s)", this, id, world, x, y, z));
    ...
    Container getGuiContainer(int id, EntityPlayer player, World world, BlockPos pos, int param) {
        ...
        if (debugGui) SGCraft.log.debug(String.format("%s: BaseMod.getGuiContainer: No Container class found for gui id %d", this, id));
        return null;
    }
    ...
    Object createGuiElement(Class cls, EntityPlayer player, World world, BlockPos pos, int param) {
        try {
            if (debugGui) SGCraft.log.debug(String.format("BaseMod.createGuiElement: Looking for create method on %s for %s in %s", cls, player, world));
            ...
            if (debugGui) SGCraft.log.debug(String.format("BaseMod.createGuiElement: Looking for constructor on %s", cls));
            ...
            throw new RuntimeException(String.format("%s: No suitable gui element constructor found for %s", modID, cls));
        }
        catch (Exception e) {

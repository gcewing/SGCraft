...
public class BaseModClient<MOD extends BaseMod<? extends BaseModClient>> implements IGuiHandler {
    ...
    protected void registerDefaultRenderers() {
        for (Block block : base.registeredBlocks) {
            ...
            if (block instanceof IBlock) {
                if (debugModelRegistration)
                    SGCraft.log.debug(String.format("BaseModClient.registerDefaultRenderers: %s %s", block, block.getUnlocalizedName()));
        ...
        for (Item item : base.registeredItems) {
            if (debugModelRegistration)
                SGCraft.log.debug(String.format("BaseModClient.registerDefaultRenderers: %s %s", item, item.getUnlocalizedName()));
            if (itemNeedsCustomRendering(item))
                installCustomItemRenderDispatcher(item);
        }
    }
    ...
    protected void removeUnusedDefaultTextureNames() {
        for (Block block : base.registeredBlocks) {
            if (blockNeedsCustomRendering(block)) {
                if (debugModelRegistration)
                    SGCraft.log.debug(String.format("BaseModClient: Removing default texture from block %s", block.getUnlocalizedName()));
        ...
        for (Item item : base.registeredItems) {
            if (itemNeedsCustomRendering(item)) {
                if (debugModelRegistration)
                    SGCraft.log.debug(String.format("BaseModClient: Removing default texture from item %s", item.getUnlocalizedName()));
     ...
     * On the server side, this needs to return an instance of Container
     *
    ...
    public Object getClientGuiElement(int id, EntityPlayer player, World world, BlockPos pos) {
        ...
        if (scrnCls != null) {
            if (base.debugGui) SGCraft.log.debug(String.format("BaseModClient.getClientGuiElement: Instantiating %s", scrnCls));
            ...
            if (contCls != null) {
                try {
                    if (base.debugGui) SGCraft.log.debug(String.format("BaseModClient.getClientGuiElement: Looking for constructor taking %s", contCls));
                    ...
                    if (cont != null) {
                        if (base.debugGui) SGCraft.log.debug("BaseModClient.getClientGuiElement: Instantiating screen with container");
                        try {
    ...
    protected class BlockRenderDispatcher implements ISimpleBlockRenderingHandler {
        ...
        public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks rb) {
            ...
            if (debugRenderBlock) SGCraft.log.debug(String.format("BaseModClient.BlockRenderDispatcher.renderWorldBlock: %s with meta %s state %s", block, meta, state));
            ICustomRenderer renderer = getCustomBlockRenderer(world, pos, state);
            if (renderer != null) {
                if (debugRenderBlock) SGCraft.log.debug(String.format("BaseModClient.BlockRenderDispatcher.renderWorldBlock: using %s", renderer));
                int pass = ForgeHooksClient.getWorldRenderPass();
                for (EnumWorldBlockLayer layer : passLayers[pass + 1]) {
                    if (debugRenderBlock) SGCraft.log.debug(String.format("BaseModClient.BlockRenderDispatcher.renderWorldBlock: %s in layer %s", block, layer));
    ...
    protected class ItemRenderDispatcher implements IItemRenderer {
        ...
        public void renderItem(ItemRenderType type, ItemStack stack, Object... data) {
            if (debugRenderItem) SGCraft.log.debug(String.format("BaseModClient.ItemRenderDispatcher.renderItem: %s %s pass %s", type, stack, MinecraftForgeClient.getRenderPass()));
            ICustomRenderer renderer = itemRenderers.get(stack.getItem());
            if (debugRenderItem) SGCraft.log.debug(String.format("BaseModClient.ItemRenderDispatcher.renderItem: Custom renderer = %s", renderer));
            if (renderer == null) {
                renderer = getModelRendererForItemStack(stack);
                if (debugRenderItem) SGCraft.log.debug(String.format("BaseModClient.ItemRenderDispatcher.renderItem: Model renderer = %s", renderer));
            }
            if (renderer != null) {
                Trans3 t;
                switch (type) {
    ...
    protected ICustomRenderer getModelRendererForItemStack(ItemStack stack) {
        ...
        if (debugRenderItem) SGCraft.log.debug(String.format("BaseModClient.getModelRendererForItemStack: item = %s %s", item, item.getUnlocalizedName()));
        ...
        if (item instanceof ItemBlock) {
            ...
            if (debugRenderItem) SGCraft.log.debug(String.format("BaseModClient.getModelRendererForItemStack: block = %s %s", block, block.getUnlocalizedName()));
    ...
    public void renderItemStackUsingModelSpec(ItemStack stack, IRenderTarget target, Trans3 t) {
        ...
        if (debugRenderItem) SGCraft.log.debug(String.format("BaseModClient.renderItemStackUsingModelSpec: renderer = %s", rend));
        if (rend != null)

...
public class OCWirelessEndpoint implements WirelessEndpoint {
    ...
    public void receivePacket(Packet packet, WirelessEndpoint sender) {
        if (debugWireless) SGCraft.log.debug(String.format("OCSGWirelessEndpoint.receivePacket: ttl %s from %s by %s", packet.ttl(), sender, te));
        if (packet.ttl() > 0) {
            SGBaseTE dte = te.getConnectedStargateTE();
            if (dte != null && dte.ocWirelessEndpoint != null) {
                if (debugWireless) SGCraft.log.debug(String.format("OCSGWirelessEndpoint.receivePacket: forwarding to %s", dte));

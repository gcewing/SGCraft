...
public class OCInterfaceTE extends SGInterfaceTE implements IComputerInterface, Environment, IInventory, ITickable {
    ...
    public void rebroadcastNetworkPacket(Object packet) {
        if (packet instanceof Packet && hasNetworkCard()) {
            if (node != null) {
                if (debugNetworking) SGCraft.log.debug("OCInterfaceTE.rebroadcastNetworkPacket");
    ...
    public void onConnect(final Node node) {
        ...
        if (debugConnection) SGCraft.log.debug(String.format("OCInterfaceTE.onConnect: %s", node));
    }
    ...
    public void onDisconnect(final Node node) {
        ...
        if (debugConnection) SGCraft.log.debug(String.format("OCInterfaceTE.onDisconnect: %s", node));
    }

...
//      public static IProperty FACING = PropertyDirection.create("facing", Plane.HORIZONTAL);
            ...
            float hitX, float hitY, float hitZ, IBlockState baseState, EntityLivingBase placer)
        {
            ...
            if (debugPlacement) SGCraft.log.debug(String.format("BaseOrientation.Orient4WaysByState: Placing block with FACING = %s", dir));
        ...
        public Trans3 localToGlobalTransformation(IBlockAccess world, BlockPos pos, IBlockState state, Vector3 origin) {
            ...
            if (debugOrientation) SGCraft.log.debug(String.format("BaseOrientation.Orient4WaysByState.localToGlobalTransformation: for %s: facing = %s", state, f));
            int i;
            switch (f) {

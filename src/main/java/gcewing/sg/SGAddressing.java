...
public class SGAddressing {
    ...
    protected static boolean validSymbols(String s) {
        for (int i = 0; i < s.length(); i++) if (charToSymbol(s.charAt(i)) < 0) return false;
        return true;
    }

    protected static void validateAddress(String s) throws AddressingError {
        int l = s.length();
        if ((l == numCoordSymbols || l == numCoordSymbols + numDimensionSymbols) && validSymbols(s)) return;
    ...
    public static String addressForLocation(SGLocation loc) throws AddressingError {
        ...
        int d = hash(dp, pd, md);
        if (debugAddressing) SGCraft.log.debug(
                String.format(
    ...
    public static SGBaseTE findAddressedStargate(String address, World fromWorld) throws AddressingError {
        ...
        if (debugAddressing) SGCraft.log
                .debug(String.format("SGAddressing.findAddressedStargate: c = %s chunk = (%d,%d)", c, chunkX, chunkZ));
        SGBaseTE te = null;
        if (address.length() == maxAddressLength) {
            ...
            if (debugAddressing)
                SGCraft.log.debug(String.format("SGAddressing.findAddressedStargate: d = %s dimension = %s", d, dm));
            if (dm != null) te = getBaseTE(chunkX, chunkZ, dm);
            if (te == null) {
                // Try old interpretation of dimension symbols
                int dimOld = minDirectDimension + hash(d, qdOld, mdOld);
                if (debugAddressing) SGCraft.log
                        .debug(String.format("SGAddressing.findAddressedStargate: Trying dimension = %s", dimOld));
    ...
    protected static long interleaveCoords(int x, int z) {
        ...
        while (x > 0 || z > 0) {
            if (debugAddressing)
                SGCraft.log.debug(String.format("SGAddressing.interleaveCoords: half-digits %d %d", x % 6, z % 6));
            c += p6 * (x % 6);
            x /= 6;
            p6 *= 6;
            c += p6 * (z % 6);
    ...
    public static String padAddress(String address, String caret, int maxLength) {
        if (maxLength < numCoordSymbols) maxLength = numCoordSymbols;
    ...
    public static String formatAddress(String address, String sep1, String sep2) {
        ...
        if (dimen.length() > 0) result += sep2 + dimen;
        return result;
    }

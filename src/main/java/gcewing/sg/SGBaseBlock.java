...
public class SGBaseBlock extends SGBlock<SGBaseTE>  {
    ...
    {
        if (!world.isRemote) SGCraft.log.debug(String.format("SGBaseBlock: at %s meta %s state %s", pos, world.getBlockMetadata(pos.x, pos.y, pos.z), state));
        String Side = world.isRemote ? "Client" : "Server";
        SGBaseTE te = getSGBaseTE(world, pos);
        if (te != null) {
            if (debugMerge) SGCraft.log.debug(String.format("SGBaseBlock.onBlockActivated: %s: isMerged = %s", Side, te.isMerged));
            if (te.isMerged) {
            ...
            for (int i = -2; i <= 2; i++)
                for (int j = 0; j <= 4; j++)
                    if (!(i == 0 && j == 0)) {
                        ...
                        if (pat != 0 && type != pat) {
                            if (debugMerge) SGCraft.log.debug(String.format("SGBaseBlock: world %d != pattern %d at %s", type, pattern[j][2 + i], rp));
                            return;
                        }
                    }
    ...
    void unmergeRingBlock(World world, BlockPos pos, BlockPos ringPos) {
        ...
        if (debugMerge) SGCraft.log.debug(String.format("SGBaseBlock.unmergeRingBlock: found %s at %s", block, ringPos));
        if (block instanceof SGRingBlock) {
